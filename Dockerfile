FROM docker.io/rclone/rclone:1.74.2 AS rclone
FROM docker.io/kopia/kopia:0.23.0 AS kopia
FROM docker.io/restic/restic:0.18.1 AS restic
FROM ghcr.io/rustic-rs/rustic:v0.11.2 AS rustic
...
RUN  apk add --update --no-cache task just libwebp-tools wget curl jq yq xz mariadb-client openssh-client midnight-commander rsync grype regclient croc
